...
class AIPlayer(Player):
    ...
    def choose_attack_card(self, attack_card: Card, trump: Suit) -> Card | None:
        """
        Pick the cheapest card that beats the attack card.
        Prefer same-suit beats; use trump only as last resort.
        """
        same_suit = [c for c in self.hand if c.suit == attack_card.suit and c.beats(attack_card, trumo)]
        if same_suit:
